...
export default function MarketplacePage({ showToast }) {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const mapped = data.map(p => ({
          ...p,
          id: p._id, // Ensure id is set for components expecting it
          category: TYPE_MAP[p.type] || 'Other',
          rating: 5, // Backend doesn't have rating yet, default to 5
          seller: 'Local Farmer', // Default seller type
          location: p.region,
          stock: p.quantity // Map backend quantity to frontend stock
        }));
        setProducts(mapped);
      } catch (err) {
  ...
  const filtered = useMemo(() => {
    let result = products.filter(p => {
      ...
      // Ratings and sellerTypes filtering might need more backend support later
      return true;
    });
